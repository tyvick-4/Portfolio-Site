# The Server Admin Problem Nobody Is Solving

*A customer-first take on Discord's biggest untapped growth opportunity.*

---

## Start with a server admin, not a search algorithm
...
## What the server admin actually needs

The job isn't "get more Discord members." That's an output. The job is:
...
A guided setup flow that helps admins configure their forums for public indexability, with moderation defaults and trust-and-safety rails built in. Structured metadata (`DiscussionForumPosting` schema) that signals to search engines: this is a real community thread, here's the topic, here's the engagement. Then bring the signal back to admins in their Growth & Activation view — which external sources are driving joins, which forum threads are getting picked up.
...
The north star isn't member count. It's:

**Externally-referred server joins retained at 7 days.**
...
*Tyler Vick is a product manager focused on growth, community platforms, and AI-era search.*
